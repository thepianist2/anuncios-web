...
<h1 align = "center"> Libreria de archivos </h1>

<p>Es una librería de archivos donde se puede Crear carpetas para clasificar los archivos que debemos almacenar para la publicación de los contenidos.</p>
<br />
<div id="sf_media_browser_user_message"></div>

<div id="sf_media_browser_forms">
  <fieldset id="sf_media_browser_upload">
    <legend><?php echo __('Subir archivos menores de 2Mb') ?></legend>
    <form action="<?php echo url_for('sf_media_browser_file_create') ?>" method="post" enctype="multipart/form-data">
      <?php echo $upload_form ?>
      <input type="submit" class="submit" value="<?php echo __('Guardar') ?>" />
    </form>
  </fieldset>

  <fieldset id="sf_media_browser_mkdir">
    <legend><?php echo __('Crear un nuevo directorio') ?></legend>
    <form action="<?php echo url_for('sf_media_browser_dir_create') ?>" method="post">
      <?php echo $dir_form ?>
      <input type="submit" class="submit" value="<?php echo __('Crear') ?>" />
    </form>
  </fieldset>
  <div class="clear"></div>
</div>
